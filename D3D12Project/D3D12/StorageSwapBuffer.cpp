...
StorageSwapBuffer::StorageSwapBuffer(ID3D12Device* pDevice, unsigned int totalSize, unsigned int stride)
{
    for (unsigned int i = 0; i < mBufferCount; ++i)
        mBuffers[i] = new StorageBuffer(pDevice, totalSize, stride);
}
